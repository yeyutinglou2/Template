C205108.event.ready = function () {
    console.log("[C205108].ready");
    /* console.log('[C205108].ad: ' + JSON.stringify(kitex.data)); */
    C205108.event.lottieWidgetEvent(C205108.lottie_widget);
};

C205108.event.viewableChange = function (viewable) {
    if (C205108.motion != null) {
        if (viewable) {
            C205108.motion.start();
        } else {
            C205108.motion.stop();
        }
    }
...
C205108.event.makeNode = function (params) {
    if (params.type == "LottieView") {
        if (params.nodeId == "widget_id") {
            C205108.lottie_widget = new kitex.Lottie(params);
        }
        return;
    }
};

C205108.event.widgetId = function () {
    return C205108.widgetId;
};

C205108.event.lottieWidgetEvent = function (lottie) {
    let widgetId = C205108.event.widgetId();
    lottie.addEventListener("animationLoaded", function (event) {
        lottie.play(0, 1, 0);
        C205108.event.interactiveEvent(lottie);
    });
...
/* 互动事件处理 */
C205108.event.interactiveEvent = function (lottie) {
    /* 互动挂件 */
    /* console.log('[C205108].ad: ' + JSON.stringify(kitex.data)); */
    let ad = kitex.data.ads[0];
    let material = ad.materials[0];
    ...
    if (material.click_type == 1) {
        ...
        clickAreas.forEach((val, index) => {
            lottie.addClick(val, function (params) {
                ...
                params.dcParams.cpt_id = C205108.widgetId;
                params.dcParams.accpt_ids = C205108.widgetId;
                console.log(
                    "[C205108].widget.click: " + JSON.stringify(params)
                );
...
C205108.event.motionEvent = function (motion) {
    motion.start();
    motion.addEventListener("start", function (params) {
        console.log("[C205108].motion.start: " + JSON.stringify(params));
    });

    motion.addEventListener("end", function (params) {
        params.sld = C205108.sld;
    ...
    motion.addEventListener("progress", function (params) {
        console.log("[C205108].motion.progress: " + JSON.stringify(params));
    });
};